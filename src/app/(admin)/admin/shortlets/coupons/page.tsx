...
export default function AdminShortletCouponsPage() {
  ...
  const load = () => {
    fetch("/api/admin/shortlet-coupons").then((r) => r.json()).then((d) => { setCoupons(Array.isArray(d) ? d : []); setLoading(false); });
  };
  useEffect(load, []);
  ...
  const create = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/admin/shortlet-coupons", {
      method: "POST",
  ...
  const toggleActive = async (c: Coupon) => {
    await fetch(`/api/admin/shortlet-coupons/${c.code}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const remove = async (code: string) => {
    ...
    await fetch(`/api/admin/shortlet-coupons/${code}`, { method: "DELETE" });
    load();
  };
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <Link href="/admin/shortlets" style={{ display: "flex", alignItems: "center", gap: 5, color: "#aaa", fontSize: 12, textDecoration: "none", marginBottom: 14 }}>
          <ArrowLeft size={12} /> Shortlets
        </Link>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <p style={{ color: ORANGE, fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", margin: "0 0 6px" }}>ABODE</p>
            <h1 style={{ color: "#0a0a0a", fontSize: 24, fontWeight: 700, margin: 0 }}>Coupons</h1>
          </div>
          <button
            onClick={() => setShowForm((v) => !v)}
            style={{ display: "flex", alignItems: "center", gap: 8, background: ORANGE, color: "#fff", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
          >
        ...
        <form onSubmit={create} style={{ background: "#fff", border: "1px solid #e8e8e4", borderRadius: 14, padding: 20, marginBottom: 20 }}>
          <div style={{ display: "flex", gap: 12, marginBottom: 14 }}>
            <div style={{ flex: 1 }}>
              ...
              <input style={{ ...inputStyle, textTransform: "uppercase" }} value={form.code} onChange={(e) => setForm((p) => ({ ...p, code: e.target.value }))} placeholder="STAY10" required />
            </div>
            <div style={{ flex: 1 }}>
          ...
          <button type="submit" disabled={saving} style={{ background: ORANGE, color: "#fff", border: "none", borderRadius: 10, padding: "10px 20px", fontSize: 13, fontWeight: 600, cursor: "pointer", opacity: saving ? 0.6 : 1 }}>
                ...
                <tr key={c.code} style={{ borderBottom: "1px solid #f5f5f5" }}>
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    <button
                      ...
                      style={{ background: c.active ? ORANGE_BG : "#f5f5f3", color: c.active ? ORANGE : "#999", border: "none", borderRadius: 999, padding: "4px 12px", fontSize: 11, fontWeight: 600, cursor: "pointer" }}
                    >
