...
export default function EditShortletPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/shortlets/${id}`).then((r) => r.json()).then((data) => {
      setForm({ title: data.title ?? "", location: data.location ?? "", city: data.city ?? "Lagos", price: String(data.price ?? ""), bedrooms: String(data.bedrooms ?? "1"), bathrooms: String(data.bathrooms ?? "1"), guests: String(data.guests ?? "2"), amenities: data.amenities ?? [], tags: (data.tags ?? []).join(", "), images: (data.images ?? []).join(", "), host_name: data.host?.name ?? "", available: data.available ?? true });
      setLoading(false);
    });
  }, [id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { title: form.title, location: form.location, city: form.city, price: parseInt(form.price), bedrooms: parseInt(form.bedrooms), bathrooms: parseInt(form.bathrooms), guests: parseInt(form.guests), amenities: form.amenities, tags: form.tags.split(",").map((t) => t.trim()).filter(Boolean), images: form.images.split(",").map((u) => u.trim()).filter(Boolean), host: { name: form.host_name || "Studio Mudiaga", avatar: "", superhost: false }, available: form.available };
    const res = await fetch(`/api/admin/shortlets/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    ...
    <div style={{ maxWidth: 720 }}>
      ...
      <form onSubmit={handleSubmit}>
        <Section title="Basic Info">
          ...
          <Row>
            <Field label="Location *"><input style={inputStyle} value={form.location} onChange={(e) => set("location", e.target.value)} required /></Field>
          ...
          <Field label="Price per night (₦) *"><input style={inputStyle} type="number" value={form.price} onChange={(e) => set("price", e.target.value)} required min="0" /></Field>
        </Section>
        <Section title="Details">
          ...
          <Field label="Host Name"><input style={inputStyle} value={form.host_name} onChange={(e) => set("host_name", e.target.value)} /></Field>
        </Section>
        ...
        <Section title="Images & Tags">
          <Field label="Image URLs (comma-separated)"><textarea style={{ ...inputStyle, minHeight: 72, resize: "vertical" }} value={form.images} onChange={(e) => set("images", e.target.value)} /></Field>
          <Field label="Tags (comma-separated)"><input style={inputStyle} value={form.tags} onChange={(e) => set("tags", e.target.value)} /></Field>
        </Section>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button type="submit" disabled={saving} style={{ background: NAVY, color: "#fff", border: "none", borderRadius: 10, padding: "11px 24px", fontSize: 13, fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}>{saving ? "Saving…" : "Save Changes"}</button>
