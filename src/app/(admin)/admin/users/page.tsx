...
import { Users, Search, ShoppingBag, CalendarDays } from "lucide-react";
...
export default function AdminUsersPage() {
  ...
  const totalOrderSpend = users.reduce((n, u) => n + u.total_spent, 0);
  const totalBookingSpend = users.reduce((n, u) => n + u.booking_spent, 0);
  const withOrders = users.filter((u) => u.order_count > 0).length;
  const withBookings = users.filter((u) => u.booking_count > 0).length;

  return (
    <div style={{ padding: "32px 28px", minHeight: "100vh", backgroundColor: "#fafafa" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          <p style={{ fontSize: 20, fontWeight: 700, color: "#1e156d", margin: 0 }}>Customers</p>
          <p style={{ fontSize: 13, color: "#888", margin: "4px 0 0" }}>{users.length} registered account{users.length !== 1 ? "s" : ""} across MUDRES and ABODE</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 24 }}>
        {[
          ...
          { icon: ShoppingBag, label: "Have ordered furniture", value: withOrders, color: "#10b981" },
          { icon: CalendarDays, label: "Have booked a stay", value: withBookings, color: "#c46442" },
          { icon: ShoppingBag, label: "Total revenue", value: fmt(totalOrderSpend + totalBookingSpend), color: "#2A3812" },
      ...
      <div style={{ background: "#fff", border: "1px solid #eee", borderRadius: 14, overflow: "hidden", overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: "1px solid #f0f0f0", backgroundColor: "#fafafa" }}>
              {["Customer", "Joined", "Orders", "Order Spend", "Bookings", "Booking Spend"].map((h) => (
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} style={{ textAlign: "center", padding: 48, color: "#aaa" }}>Loading customers…</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={6} style={{ textAlign: "center", padding: 48, color: "#aaa" }}>No customers found</td></tr>
            ) : (
              filtered.map((u, i) => (
                <tr key={u.id} style={{ borderBottom: i < filtered.length - 1 ? "1px solid #f5f5f5" : "none" }}>
                  ...
                  <td style={{ padding: "14px 16px", color: "#888", fontSize: 12, whiteSpace: "nowrap" }}>{fmtDate(u.created_at)}</td>
                  <td style={{ padding: "14px 16px", color: "#333", fontWeight: 600 }}>{u.order_count}</td>
                  <td style={{ padding: "14px 16px", color: "#1e156d", fontWeight: 600, whiteSpace: "nowrap" }}>{fmt(u.total_spent)}</td>
                  <td style={{ padding: "14px 16px", color: "#333", fontWeight: 600 }}>{u.booking_count}</td>
                  <td style={{ padding: "14px 16px", color: "#c46442", fontWeight: 600, whiteSpace: "nowrap" }}>{fmt(u.booking_spent)}</td>
                </tr>
              ))
            )}
          </tbody>
