...
export default function NewFurniturePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { name: form.name, category: form.category, material: form.material, price: parseInt(form.price), original_price: form.original_price ? parseInt(form.original_price) : null, currency: "NGN", description: form.description, dimensions: form.dimensions, weight: form.weight, colors: colorList, images: form.images, tags: form.tags.split(",").map((t) => t.trim()).filter(Boolean), in_stock: form.in_stock, featured: form.featured, product_of_month: form.product_of_month, rating: 0, review_count: 0 };
    ...
    if (!res.ok) { setError(resBody.error ?? "Failed to save"); setSaving(false); return; }
    const { id } = resBody;
