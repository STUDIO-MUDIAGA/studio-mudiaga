...
export async function PUT(req: Request, { params }: { params: Promise<{ code: string }> }) {
  ...
  const { error } = await db.from("furniture_coupons").update(patch).eq("code", decodeURIComponent(code).toUpperCase());
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ code: string }> }) {
  const { code } = await params;
  const { error } = await db.from("furniture_coupons").delete().eq("code", decodeURIComponent(code).toUpperCase());
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ success: true });
}
