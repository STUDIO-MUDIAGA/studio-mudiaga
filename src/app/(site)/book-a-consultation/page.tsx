...
import { Check, X, Loader2, Upload, ArrowRight } from "lucide-react";
...
function TextArea({ rows = 6, ...props }: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      {...props}
      rows={rows}
      style={{ ...inputStyle, resize: "vertical", lineHeight: 1.7, minHeight: rows * 24 }}
      onFocus={(e) => (e.currentTarget.style.borderColor = "rgba(251,191,36,0.4)")}
      onBlur={(e) => (e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)")}
    />
  );
...
export default function BookAConsultationPage() {
  ...
  return (
    <div style={{ background: "#0a0a0a", minHeight: "100dvh", boxSizing: "border-box", paddingTop: 96 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Section eyebrow="Your Vision">
            ...
            <Field label="How do you live in your space day to day?" hint="Describe a typical morning, evening, or weekend at home.">
              <TextArea rows={6} value={dailyLife} onChange={(e) => setDailyLife(e.target.value)} placeholder="Walk us through a normal day…" />
            </Field>
          ...
          <Section eyebrow="Your Expectations">
            ...
            <Field label="Is there anything specific about your lifestyle, work, or personality that you think is important for us to understand before we begin?">
              <TextArea rows={6} value={importantContext} onChange={(e) => setImportantContext(e.target.value)} placeholder="Whatever feels relevant…" />
            </Field>
            <Field label="Is there anything else you would like us to know?">
              <TextArea rows={6} value={anythingElse} onChange={(e) => setAnythingElse(e.target.value)} placeholder="The floor is yours…" />
            </Field>
          </Section>

          {error && (
            <div style={{ marginTop: 32, color: "#f87171", fontSize: 12, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 10, padding: "10px 14px" }}>
              {error}
            </div>
          )}
          ...
          <div style={{ marginTop: 40 }}>
            <button
              ...
              style={{
                ...
                cursor: submitting || stillUploading ? "not-allowed" : "pointer",
