...
const REVEAL_EASE = [0.215, 0.61, 0.355, 1] as [number, number, number, number];

const LINKS = ["Home", "About", "Projects", "Gallery", "Booking"];
...
function MenuToggle({ open, onClick }: { open: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="relative h-11 w-28 rounded-full text-white text-sm tracking-wide cursor-pointer"
      style={{
        backgroundColor: "rgba(255,255,255,0.08)",
        border: "1px solid rgba(255,255,255,0.16)",
      }}
    >
      <span className="relative block h-full w-full" style={{ perspective: "6em" }}>
        <motion.span
          ...
          transition={{
            transform: { duration: 0.55, ease: [0.645, 0.045, 0.355, 1] },
            opacity: { duration: 0.35, ease: "linear", delay: open ? 0 : 0.2 },
          }}
        >
          Menu
        </motion.span>
        <motion.span
          ...
          transition={{
            transform: { duration: 0.75, ease: [0.645, 0.045, 0.355, 1] },
            opacity: { duration: 0.35, ease: "linear", delay: open ? 0.3 : 0 },
          }}
        >
          Close
        </motion.span>
...
function MenuItem({
  ...
  return (
    <motion.a
      ...
      transition={{ duration: 0.5, ease: REVEAL_EASE, delay: 0.2 + index * 0.06 }}
    >
      {label}
    </motion.a>
  );
...
function SocialLink({ href, label }: { href: string; label: string }) {
  return (
    <a
      ...
      className="text-sm tracking-wide text-white/40 hover:text-white/80 transition-colors"
    >
      {label}
    </a>
  );
...
export default function HeaderV2({ revealed }: { revealed: boolean }) {
  ...
  return (
    <>
      <motion.div
        className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-10 py-6 md:py-8"
        animate={revealed ? { opacity: barHidden ? 0 : 1, y: 0 } : {}}
        initial={{ opacity: 0, y: -16 }}
        transition={{ duration: 0.3, ease: EASE }}
        style={{ pointerEvents: barHidden ? "none" : "auto" }}
      >
        <LogoMark className="text-white h-4 w-auto" />
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            className="fixed top-20 right-6 md:top-24 md:right-10 z-40 w-64 rounded-3xl overflow-hidden flex flex-col"
            style={{
          ...
          >
            <nav className="flex flex-col items-start gap-0.5 px-6 pt-6 pb-4">
              {LINKS.map((link, i) => (
                <MenuItem key={link} label={link} index={i} onClick={() => setOpen(false)} />
              ))}
            </nav>
